...
int max_wanted_pos, min_wanted_pos, max_A_pot, min_A_pot; //, max_A_pot_temp;
...
// TAL VEZ ANTES DE PARAR AL MOTOR, LLEVARLO A LA SAFE POSITION
          // Make sure the motor stops after PID callibration
...
void set_adj_v_values()
{
  ...
  if(millis() - lastUSRblink > DELTA_LCD_REFRESH)
  {
    ...
    // lcd.print(Compression_perc);
    // lcd.print(" Pot: ");
// //   lcd.print(int(map(max_A_pot_temp, min_arm_pos, max_arm_pos, 0, 100)));
    // lcd.print(int(map(A_pot, min_arm_pos, max_arm_pos, 0, 100)));
    // lcd.setCursor(0, 1);
    // lcd.print("adj_v %: ");
    // lcd.print(adj_v[i]);
#endif
